...
export default function FollowingVotesSection({
  ...
  const { data: followingVotes = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Get votes from followed users on this poll
  ...
  if (!user || isLoading || followingVotes.length === 0) return null;
    ...
    <div className="mt-3 p-3 rounded-xl bg-secondary/50 border border-border/40">
      ...
      {agreeing.length > 0 && (
        <p className="text-[10px] text-muted-foreground mt-2">
          ...
          {agreeing.length === 1 ? 'person' : 'people'} you follow agree with
          you
          {disagreeing.length > 0 && (
            <>
              {' · '}
              <span className="font-bold">{disagreeing.length}</span> chose the
              other side
            </>
          )}
        </p>
