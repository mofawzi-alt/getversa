...
export default function PulseStoriesRow() {
  ...
  const circles: CircleSpec[] = useMemo(() => {
    ...
    const emptyCard = (label: string, msg: string): StoryCardData => ({
      label,
      headline: msg,
      primaryText: 'Check back soon',
      secondaryText: '',
    });

    // ── 1) Egypt Today (always first)
    {
      const hasData = settings?.egypt_today_enabled !== false && (pulse?.egypt_today?.length || 0) > 0;
      const seen = hasSeenLocally('egypt_today');
      list.push({
        topic: 'egypt_today',
        label: 'Egypt Today',
        cards: hasData
          ? pulse!.egypt_today.map((c, i) =>
              pulseCardToStory(c, i === 0 ? 'Egypt Today' : 'Today in Egypt', emojiFor(c.category || ''))
            )
          : [emptyCard('Egypt Today', 'No updates yet today')],
        dot: hasData ? (seen ? null : 'blue') : null,
        priority: 0,
        fixedPosition: 0,
      });
    }
    ...
    // ── 2) Battle of the Day (always second)
    {
      if (battleData) {
        ...
        list.push({
          topic: 'battle',
          label: 'Battle',
          cards: [emptyCard('Battle', 'No battle today yet')],
          dot: null,
          priority: 1,
          fixedPosition: 1,
        });
    ...
    {
      const hasData = user && updatesData && updatesData.length > 0;
      list.push({
        topic: 'updates',
        label: 'Updates',
        cards: hasData
          ...
          : [emptyCard('Updates', 'Vote on polls to see updates here')],
        dot: hasData ? 'blue' : null,
        priority: 2,
        fixedPosition: 2,
      });
    }
    ...
    // ── Closing Soon
    {
      const hasData = user && closingData && closingData.length > 0;
      list.push({
        topic: 'closing_soon',
        label: 'Closing Soon',
        cards: hasData
          ? closingData.map((c: any) => ({
          ...
          : [emptyCard('Closing Soon', 'No polls closing soon')],
        dot: hasData ? 'red' : null,
        priority: 10,
      });
    }
    ...
    // ── Cairo Now
    {
      const hasData = settings?.cairo_enabled !== false && (pulse?.cairo?.length || 0) > 0;
      ...
      list.push({
        ...
        cards: hasData
          ...
          : [emptyCard('Cairo Now', 'No Cairo divergence today')],
        dot: hasData ? (seen ? null : 'blue') : null,
        priority: 20,
      });
    }
    ...
    // ── New This Week
    {
      const hasData = user && newPollsData && newPollsData.length >= 1;
      list.push({
        topic: 'new_this_week',
        label: 'New',
        cards: hasData
          ...
          : [emptyCard('New', 'No new polls this week yet')],
        dot: hasData ? 'blue' : null,
        priority: 30,
      });
    }
    ...
    // ── Your Categories
    {
      const hasData = categoriesData && categoriesData.length > 0;
      list.push({
        topic: 'categories',
        label: 'For You',
        cards: hasData
          ...
          : [emptyCard('For You', 'Vote more to unlock personalized stories')],
        dot: hasData ? (hasSeenLocally('categories') ? null : 'blue') : null,
        priority: 40,
      });
    }
    ...
    // ── Predict Results
    {
      list.push({
        ...
        cards: predictData
          ...
          : [emptyCard('Predictions', 'Play Predict to see results here')],
        dot: predictData ? (hasSeenLocally('predict') ? null : 'blue') : null,
        priority: 50,
      });
    }
    ...
    // ── Friends Activity
    {
      const hasData = user && friendsData && friendsData.length > 0;
      list.push({
        topic: 'friends',
        label: 'Friends',
        cards: hasData
          ...
          : [emptyCard('Friends', 'Add friends to see their activity')],
        dot: hasData ? 'gold' : null,
        priority: 60,
      });
    }
    ...
    // ── Weekly Verdict
    {
      const hasData = weeklyData && weeklyData.top5.length > 0;
      if (hasData) {
        ...
        list.push({
          topic: 'weekly',
          label: 'Weekly',
          cards: [emptyCard('Weekly', 'Weekly verdict drops on Sunday')],
          dot: null,
          priority: 5,
          goldBorder: false,
        });
    ...
    // ── The Breakdown
    {
      const hasData = breakdownData && breakdownData.length > 0;
      list.push({
        topic: 'breakdown',
        label: 'Breakdown',
        cards: hasData
          ? breakdownData.map((f) => breakdownToCard(f))
          : [emptyCard('Breakdown', 'No demographic breakdowns yet')],
        dot: hasData ? (hasSeenLocally('breakdown') ? null : 'blue') : null,
        priority: 35,
      });
    }
    ...
    // ── Morning Recap
    {
      const hasRecap = settings?.morning_pulse_enabled !== false && pulse?.cards;
      ...
      list.push({
        topic: 'morning_recap',
        label: 'Recap',
        cards: recapCards.length > 0 ? recapCards : [emptyCard('Recap', 'Morning recap coming soon')],
        dot: recapCards.length > 0 ? (hasSeenLocally('morning_recap') ? null : 'blue') : null,
        priority: 4,
      });
    }
