...
export default function SuggestPollButton({ question, askQueryId, variant = 'card' }: Props) {
  ...
  const submit = async () => {
    if (!user) { toast.error(t('Sign in to suggest polls')); return; }
    ...
    try {
      ...
      setSubmitted(true);
      toast.success(t('Suggestion sent! Earn +5 credits when it goes live.'));
    } catch (e: any) {
      console.error(e);
      toast.error(e?.message || t('Could not submit'));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (variant === 'compact') {
    return (
      ...
      >
        ...
        {submitted ? t('Suggested') : t('Suggest as poll')}
      </button>
    );
  }
      ...
      <div className="flex items-center gap-2.5 min-w-0">
        ...
        <div className="min-w-0">
          <p className="text-xs font-bold text-foreground">
            {submitted ? t("We'll build this poll") : t('Want us to build this poll?')}
          </p>
          <p className="text-[11px] text-muted-foreground truncate">
            {submitted ? t("You'll get +5 credits + a notification when it's live") : t('Earn +5 Ask credits when it goes live')}
          </p>
        </div>
      </div>
