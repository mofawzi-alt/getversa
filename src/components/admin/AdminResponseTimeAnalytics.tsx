...
import { Loader2, Clock, Zap, Sun, TrendingUp } from 'lucide-react';
...
export default function AdminResponseTimeAnalytics() {
  const { data, isLoading } = useQuery({
    queryKey: ['admin-response-time'],
    queryFn: async (): Promise<ResponseTimeData | null> => {
      // Get all polls (platform-wide)
    ...
    return (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-muted-foreground">No response time data yet. Create polls to track timing.</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground text-center mt-2">
            🌟 Peak hours highlighted — best time to publish polls
          </p>
        </CardContent>
      </Card>
