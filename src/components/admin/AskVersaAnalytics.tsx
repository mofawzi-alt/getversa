...
export default function AskVersaAnalytics() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Section title="Topics needing more polls (guardrail triggers)" icon={<AlertCircle className="h-4 w-4 text-amber-600" />}>
        {lowDataQuestions.length === 0 ? (
          ...
          <ul className="space-y-1.5">
            {lowDataQuestions.map((q, i) => (
              <li key={i} className="flex items-start justify-between gap-3 text-sm">
                <span className="text-foreground line-clamp-2 flex-1">{q.question}</span>
                <span className="font-bold text-amber-600 shrink-0">{q.count}×</span>
              </li>
            ))}
          </ul>
        )}
