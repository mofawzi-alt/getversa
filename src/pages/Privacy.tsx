  ...
  return (
    <div className="min-h-screen bg-white safe-area-top safe-area-bottom">
      ...
      <main className="max-w-2xl mx-auto px-5 py-8 text-foreground">
        <h1 className="text-2xl font-bold mb-1">{t('Privacy Policy')}</h1>
        ...
        <Section title={t("1. Introduction")}>
          <p>{t("Welcome to Versa (\"we\", \"our\", or \"the app\"). Versa is a consumer opinion platform that allows users to vote on binary polls, discover their personality type, and access collective insights backed by real votes.")}</p>
          <p className="mt-2">{t("This Privacy Policy explains what personal data we collect, how we use it, who we share it with, and your rights as a user. By creating an account and using Versa, you agree to this policy.")}</p>
          <p className="mt-2">{t("If you do not agree with any part of this policy, please do not use the app.")}</p>
        </Section>

        <Section title={t("2. Who We Are")}>
          <p>{t("Versa is operated by Versa Technologies. For any questions about this policy or your data, contact us at:")}</p>
        ...
        <Section title={t("3. What Data We Collect")}>
          <h3 className="font-semibold mt-2 mb-1">3.1 Account Information</h3>
          <p>{t("When you create an account, we collect:")}</p>
          ...
          <p>{t("When you complete your profile, we collect:")}</p>
          <ul className="list-disc pl-5 mt-1 space-y-0.5">
          ...
          <p className="mt-1">{t("This information is used to generate demographic breakdowns of poll results.")}</p>

          <h3 className="font-semibold mt-4 mb-1">3.3 Vote and Interaction Data</h3>
          <p>{t("When you use the app, we collect:")}</p>
          <ul className="list-disc pl-5 mt-1 space-y-0.5">
          ...
          <p>{t("We automatically collect:")}</p>
          <ul className="list-disc pl-5 mt-1 space-y-0.5">
          ...
          <p>{t("If you contact our support team, we collect the content of your message and your contact details.")}</p>
        </Section>

        <Section title={t("4. How We Use Your Data")}>
          ...
          <p>{t("We aggregate and anonymise vote data to produce consumer intelligence reports sold to brand clients (companies). These reports show demographic trends — for example, \"61% of Egyptian women aged 18–24 prefer X over Y.\"")}</p>
        ...
        <Section title={t("5. How We Share Your Data")}>
          <h3 className="font-semibold mt-2 mb-1">5.1 With Friends You Add</h3>
          <p>{t("Your username, profile photo, vote match percentage, and public stats are visible to users you connect with as friends.")}</p>

          <h3 className="font-semibold mt-4 mb-1">5.2 With Brand Clients (Anonymised Only)</h3>
          <p>{t("Aggregated, anonymised demographic breakdowns are shared with brand clients as consumer intelligence reports. No personally identifying information is ever shared.")}</p>

          <h3 className="font-semibold mt-4 mb-1">5.3 With Service Providers</h3>
          <p>{t("We use trusted third-party services to operate Versa, including:")}</p>
          <ul className="list-disc pl-5 mt-1 space-y-0.5">
            <li><strong>Supabase</strong> — database and authentication</li>
            <li><strong>AI providers</strong> — to power Ask Versa and image generation</li>
          </ul>
          <p className="mt-1">{t("These providers process data only on our behalf and under strict confidentiality obligations.")}</p>

          <h3 className="font-semibold mt-4 mb-1">5.4 Legal Requirements</h3>
          <p>{t("We may disclose your data if required by law, court order, or to protect the rights and safety of our users.")}</p>

          <h3 className="font-semibold mt-4 mb-1">5.5 Business Transfer</h3>
          <p>{t("If Versa is acquired or merges with another company, your data may be transferred as part of that transaction. We will notify you in advance.")}</p>
          <p className="mt-2">{t("We do not sell your personal data to any third party.")}</p>
        </Section>

        <Section title={t("6. Data Retention")}>
          <p>{t("We keep your data for as long as your account is active. If you delete your account:")}</p>
          <ul className="list-disc pl-5 mt-1 space-y-0.5">
        ...
        <Section title={t("7. Your Rights")}>
          <p>{t("Depending on your country, you may have the right to:")}</p>
        ...
        <Section title={t("8. Children and Age Requirements")}>
        ...
        <Section title={t("9. Security")}>
          <p>{t("We take reasonable technical and organisational measures to protect your data, including:")}</p>
          <ul className="list-disc pl-5 mt-1 space-y-0.5">
            <li>Encrypted data transmission (HTTPS)</li>
            <li>Encrypted password storage</li>
            <li>Access controls limiting who can access user data</li>
          </ul>
          <p className="mt-2">{t("No method of transmission or storage is 100% secure. We cannot guarantee absolute security, but we are committed to protecting your data.")}</p>
        </Section>

        <Section title={t("10. Cookies and Tracking")}>
          <p>{t("We use minimal tracking to keep you logged in and to understand how the app is used. We do not use advertising trackers or sell data to advertisers.")}</p>
        </Section>

        <Section title={t("11. Changes to This Policy")}>
          <p>{t("We may update this Privacy Policy from time to time. When we do, we will update the \"Last updated\" date at the top of this page. If changes are significant, we will notify you through the app or by email.")}</p>
          <p className="mt-2">{t("Your continued use of Versa after changes are posted means you accept the updated policy.")}</p>
        </Section>

        <Section title={t("12. Contact Us")}>
          <p>{t("For any questions, concerns, or requests related to your privacy:")}</p>
          <p className="mt-2"><strong>Email:</strong> <a href="mailto:privacy@getversa.app" className="text-primary underline">privacy@getversa.app</a></p>
          <p><strong>Website:</strong> <a href="https://getversa.app" className="text-primary underline">getversa.app</a></p>
        </Section>
