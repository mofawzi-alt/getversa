// Language helpers for bilingual polls (English + Arabic)

export type AppLanguage = 'en' | 'ar';

export interface BilingualPollFields {
  question: string;
  option_a: string;
  option_b: string;
  subtitle?: string | null;
  question_ar?: string | null;
  option_a_ar?: string | null;
  option_b_ar?: string | null;
  subtitle_ar?: string | null;
}

/**
 * Returns the poll text in the requested language.
 * Falls back to English whenever the Arabic field is missing.
 */
export function pollText(poll: BilingualPollFields, lang: AppLanguage) {
  const ar = lang === 'ar';
  return {
    question: ar && poll.question_ar ? poll.question_ar : poll.question,
    optionA: ar && poll.option_a_ar ? poll.option_a_ar : poll.option_a,
    optionB: ar && poll.option_b_ar ? poll.option_b_ar : poll.option_b,
    subtitle: ar && poll.subtitle_ar ? poll.subtitle_ar : (poll.subtitle ?? null),
  };
}

/** Arabic labels for the 8 fixed categories (display only; DB keeps English keys). */
export const CATEGORY_AR: Record<string, string> = {
  'Relationships': 'علاقات',
  'Money': 'فلوس',
  'Education': 'تعليم',
  'Digital Life': 'حياة رقمية',
  'Food': 'أكل',
  'Entertainment': 'ترفيه',
  'Lifestyle': 'لايف ستايل',
  'Egypt': 'مصر',
  // Legacy vote categories (still appear in taste stats)
  'Lifestyle & Society': 'لايف ستايل ومجتمع',
  'Lifestyle and Society': 'لايف ستايل ومجتمع',
  'Society & Lifestyle': 'لايف ستايل ومجتمع',
  'Social': 'اجتماعي',
  'Food & Drink': 'أكل وشرب',
  'Food & Drinks': 'أكل وشرب',
  'Food Delivery & Dining': 'أكل ومطاعم',
  'FMCG & Food': 'أكل ومنتجات',
  'FMCG and Food': 'أكل ومنتجات',
  'Finance': 'فلوس',
  'Financial Services': 'خدمات مالية',
  'Fintech & Money': 'فلوس وتكنولوجيا مالية',
  'Tech': 'تكنولوجيا',
  'Technology': 'تكنولوجيا',
  'Telco & Tech': 'اتصالات وتكنولوجيا',
  'Telecom': 'اتصالات',
  'Apps': 'تطبيقات',
  'Media & Content': 'ميديا ومحتوى',
  'Media & Entertainment': 'ميديا وترفيه',
  'Media and Entertainment': 'ميديا وترفيه',
  'Fashion': 'موضة',
  'Style & Design': 'ستايل وتصميم',
  'Beauty & Personal Care': 'جمال وعناية شخصية',
  'Fitness': 'لياقة',
  'Health & Fitness': 'صحة ولياقة',
  'Sports': 'رياضة',
  'Automotive & Mobility': 'عربيات ومواصلات',
  'Brands': 'براندات',
  'Business & Startups': 'بيزنس وشركات ناشئة',
  'E-commerce & Retail': 'تسوق أونلاين',
  'Retail & E-commerce': 'تسوق أونلاين',
  'Shopping': 'تسوق',
  'Nightlife': 'سهرات',
  'Personality': 'شخصية',
  'The Pulse': 'النبض',
};

export function categoryLabel(category: string | null | undefined, lang: AppLanguage): string {
  if (!category) return '';
  return lang === 'ar' ? (CATEGORY_AR[category] || category) : category;
}
