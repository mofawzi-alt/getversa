...
BEGIN
  ...
  -- Fresh polls top-up: only inject newly created polls IF the user is still under their daily limit.
  -- Previously this branch was uncapped and caused queues to grow without bound (Hero looked endless).
  SELECT COUNT(*) INTO _current_count
  FROM daily_poll_queues dpq
  WHERE dpq.user_id = p_user_id AND dpq.queue_date = _queue_date;
