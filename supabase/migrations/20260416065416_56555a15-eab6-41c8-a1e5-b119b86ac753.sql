...
BEGIN
  ...
  -- Create a daily seed from queue_date + user_id for consistent-within-day but different-across-days ordering
  ...
  -- Generate full queue with daily-randomized ordering
  -- Priority tiers: 1) Hot takes, 2) New today polls, 3) Shuffled by score+trending+random
  INSERT INTO daily_poll_queues (user_id, poll_id, queue_date, queue_order)
  ...
  FROM (
    ...
    WHERE p.is_active = true
      ...
      AND NOT EXISTS (SELECT 1 FROM daily_poll_queues dq WHERE dq.poll_id = p.id AND dq.user_id = p_user_id)
    ORDER BY
      -- Tier 1: Hot takes always first
      CASE WHEN p.is_hot_take = true THEN 0 ELSE 1 END,
      -- Tier 2: Polls created in last 24h get priority
      CASE WHEN p.created_at >= (now() - INTERVAL '24 hours') THEN 0 ELSE 1 END,
      -- Tier 3: Blended score with daily randomization
      -- Combine weight_score + vote popularity + daily random factor
      (
      ...
      -- Final tiebreaker: deterministic daily shuffle
      md5(p.id::text || _queue_date::text || p_user_id::text)
    LIMIT _actual_limit
  ) sub;
