...
-- Drop the broken overload created by the interrupted migration
DROP FUNCTION IF EXISTS public.generate_daily_queue(uuid, date);

-- Recreate the real function with onboarding-set support for first-time users
...
BEGIN
  ...
  -- ============================================================
  -- ONBOARDING ANCHOR SET: brand-new users (no votes yet)
  -- get the curated onboarding polls in random order per user.
  -- This runs BEFORE any normal queue logic.
  -- ============================================================
  ...
  IF NOT _has_voted AND _onboarding_count > 0 THEN
    -- Seed the queue once with the onboarding set if not already populated
    IF NOT EXISTS (
  ...
  IF EXISTS (SELECT 1 FROM daily_poll_queues dpq WHERE dpq.user_id = p_user_id AND dpq.queue_date = _queue_date) THEN
    ...
    INSERT INTO daily_poll_queues (user_id, poll_id, queue_date, queue_order)
    SELECT p_user_id, sub.id, _queue_date,
      (SELECT COALESCE(MAX(dq2.queue_order), 0) FROM daily_poll_queues dq2 WHERE dq2.user_id = p_user_id AND dq2.queue_date = _queue_date) + ROW_NUMBER() OVER (ORDER BY sub.created_at DESC)::integer
    FROM (
