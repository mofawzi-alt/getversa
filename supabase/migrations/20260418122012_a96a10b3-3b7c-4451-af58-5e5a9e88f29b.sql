-- Add visibility_mode to control how campaign polls surface to users
ALTER TABLE public.poll_campaigns
  ADD COLUMN IF NOT EXISTS visibility_mode text NOT NULL DEFAULT 'mixed';

ALTER TABLE public.poll_campaigns
  DROP CONSTRAINT IF EXISTS poll_campaigns_visibility_mode_check;

ALTER TABLE public.poll_campaigns
  ADD CONSTRAINT poll_campaigns_visibility_mode_check
  CHECK (visibility_mode IN ('mixed', 'bundle_only', 'hero_only'));

-- Update generate_daily_queue to exclude polls from bundle_only campaigns
    ...
    IF (SELECT COUNT(*) FROM daily_poll_queues dpq WHERE dpq.user_id = p_user_id AND dpq.queue_date = _queue_date) < _batch_limit THEN
      ...
      FROM (
        ...
        WHERE p.is_active = true
          ...
          AND (p.campaign_id IS NULL OR COALESCE(pc.visibility_mode, 'mixed') <> 'bundle_only')
