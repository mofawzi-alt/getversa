  ...
  IF NOT _has_voted AND _onboarding_count > 0 THEN
    IF NOT EXISTS (
      ...
      WHERE p.is_active = true
        AND NOT EXISTS (SELECT 1 FROM skipped_polls s WHERE s.poll_id = op.poll_id AND s.user_id = p_user_id);
    END IF;
    ...
    WHERE dpq.user_id = p_user_id
      ...
      AND NOT EXISTS (SELECT 1 FROM votes v WHERE v.poll_id = dpq.poll_id AND v.user_id = p_user_id)
      AND NOT EXISTS (SELECT 1 FROM skipped_polls s WHERE s.poll_id = dpq.poll_id AND s.user_id = p_user_id)
