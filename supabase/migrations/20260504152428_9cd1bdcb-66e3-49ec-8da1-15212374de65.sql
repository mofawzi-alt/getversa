...
BEGIN
  ...
  IF _current_count < _actual_limit THEN
    ...
    FROM (
      ...
      ORDER BY
        -- Age-based category weighting (40%) combined with popularity (60%)
        (CASE
          WHEN _user_age_range IS NULL THEN 0.5
          -- 18-24: boost Media, Telco, Lifestyle, Pulse; deprioritize Financial, Automotive
          WHEN _user_age_range = '18-24' THEN
            CASE p.category
              WHEN 'Media & Entertainment' THEN 0.9
              WHEN 'Telco & Tech' THEN 0.85
              WHEN 'Lifestyle & Society' THEN 0.85
              WHEN 'The Pulse' THEN 0.8
              WHEN 'FMCG & Food' THEN 0.5
              WHEN 'Beauty & Personal Care' THEN 0.5
              WHEN 'Retail & E-commerce' THEN 0.5
              WHEN 'Financial Services' THEN 0.2
              WHEN 'Automotive & Mobility' THEN 0.2
              ELSE 0.5
            END
          -- 25-34: boost Financial, Lifestyle, FMCG, Pulse; no deprioritize
          WHEN _user_age_range = '25-34' THEN
            CASE p.category
              WHEN 'Financial Services' THEN 0.8
              WHEN 'Lifestyle & Society' THEN 0.8
              WHEN 'FMCG & Food' THEN 0.75
              WHEN 'The Pulse' THEN 0.75
              WHEN 'Media & Entertainment' THEN 0.5
              WHEN 'Telco & Tech' THEN 0.5
              WHEN 'Retail & E-commerce' THEN 0.5
              WHEN 'Beauty & Personal Care' THEN 0.5
              ELSE 0.5
            END
          -- 35-44: boost Financial, FMCG, Pulse, Retail; deprioritize Beauty, Media, Telco
          WHEN _user_age_range = '35-44' THEN
            CASE p.category
              WHEN 'Financial Services' THEN 0.85
              WHEN 'FMCG & Food' THEN 0.85
              WHEN 'The Pulse' THEN 0.8
              WHEN 'Retail & E-commerce' THEN 0.75
              WHEN 'Lifestyle & Society' THEN 0.5
              WHEN 'Automotive & Mobility' THEN 0.5
              WHEN 'Beauty & Personal Care' THEN 0.25
              WHEN 'Media & Entertainment' THEN 0.25
              WHEN 'Telco & Tech' THEN 0.25
              ELSE 0.5
            END
          -- 45+: boost FMCG, Pulse, Financial; deprioritize Telco, Media, Beauty
          ELSE
            CASE p.category
              WHEN 'FMCG & Food' THEN 0.9
              WHEN 'The Pulse' THEN 0.85
              WHEN 'Financial Services' THEN 0.8
              WHEN 'Lifestyle & Society' THEN 0.5
              WHEN 'Retail & E-commerce' THEN 0.5
              WHEN 'Automotive & Mobility' THEN 0.5
              WHEN 'Telco & Tech' THEN 0.2
              WHEN 'Media & Entertainment' THEN 0.2
              WHEN 'Beauty & Personal Care' THEN 0.2
              ELSE 0.5
            END
        END) * 0.4
        + (CASE WHEN COALESCE(vc.vote_count, 0) > 0 THEN LEAST(COALESCE(vc.vote_count, 0)::float / 500.0, 1.0) ELSE 0 END) * 0.6
        DESC,
        p.created_at DESC
  ...
  -- ALWAYS inject newly-created polls (last 48h) into today's queue
  SELECT COALESCE(MAX(dpq.queue_order), 0) INTO _max_order
  FROM daily_poll_queues dpq
  WHERE dpq.user_id = p_user_id AND dpq.queue_date = _queue_date;

  INSERT INTO daily_poll_queues (user_id, poll_id, queue_date, queue_order)
  SELECT p_user_id, sub.id, _queue_date,
    _max_order + (ROW_NUMBER() OVER (ORDER BY sub.created_at DESC))::integer
  FROM (
    ...
    WHERE p.is_active = true
      ...
      AND NOT EXISTS (SELECT 1 FROM votes v WHERE v.poll_id = p.id AND v.user_id = p_user_id)
      AND NOT EXISTS (SELECT 1 FROM daily_poll_queues dq4 WHERE dq4.poll_id = p.id AND dq4.user_id = p_user_id AND dq4.queue_date = _queue_date)
  ) sub;
