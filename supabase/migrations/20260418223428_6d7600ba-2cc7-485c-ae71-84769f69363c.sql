...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM daily_poll_queues dpq WHERE dpq.user_id = p_user_id AND dpq.queue_date = _queue_date) THEN
    ...
    -- Top up batch quota when more polls are due based on time-of-day pacing
    ...
    -- Always inject any freshly-created polls (since queue was built) into the queue,
    -- regardless of batch pacing. This makes admin-created polls appear immediately.
    INSERT INTO daily_poll_queues (user_id, poll_id, queue_date, queue_order)
    SELECT p_user_id, sub.id, _queue_date,
    ...
    -- Return all queued polls (no batch_limit cap) so injected fresh polls are visible immediately
    RETURN QUERY 
